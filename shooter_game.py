...
meteor=Meteor('asteroid.png',randint(0,700),randint(0,10),4,75,100)
bullet.rect.x=-10
mixer.init()
...
def new_bullet():
    ...
    bullet.rect.x=-10
def new_meteor():
    meteor=Meteor('asteroid.png',randint(0,700),randint(0,10),10,75,100)
    meteors.add(meteor)
game=True
while game:
    ...
    window.blit(text_hits,(1,35))
    window.blit(text_lose,(1,5))
    window.blit(text_hp,(1,65))
    meteor.go()
    bullets.update()
    monsters.update()
    bullets.draw(window)
    ...
    for meteor in meteors:
        ...
        if meteor.rect.y>=600:
            ...
            new_meteor()
    if hp == 0:
        window.blit(text_final,(300,300))
        game=False
